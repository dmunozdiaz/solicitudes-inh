...
class ReporteEspecificoController extends Controller
{
    ...
    public function view(TipSolicitudes $tipSolicitudes, ReporteEspecial $repor)
    {
        ...
        return view('reportes.reporteespecifico.view')->with([
            ...
            "allPmComunas" => [],
            "allPmUsers" => [],
            "allPmProcess" => Auth::user()->tipossolictud(),
        ]);
    }

    public function listado(Request $rq) {

        $reportes = ReporteEspecial::select('id','area','descsolicitud','anio','descestado','estadoreporte')->where('user_id','=',Auth::user()->id)->limit(5)->orderBy('id', 'desc')->get();
    ...
    public function generarReporte(Request $request)
    {
        ...
        try {
            ...
            $reporte->ruta_archivo = 'reportes/especial/reporte_'.date('Ymdhis').'.xlsx';

            $reporte->save();

            DB::commit();

            $reportes = ReporteEspecial::limit(5)->orderBy('id', 'desc')->get('id')->toArray();
            
            ReporteEspecial::whereNotIn('id',$reportes)->delete();

            (new ReporteespecialExport($reporte))->queue($reporte->ruta_archivo)->chain([
